...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("BitZyon version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  BitZyond [options]                     " + "\n" +
                  "  BitZyond [options] <command> [params]  " + _("Send command to -server or BitZyond") + "\n" +
                  "  BitZyond [options] help                " + _("List commands") + "\n" +
                  "  BitZyond [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "bitzyon:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("BitZyon"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("BitZyon"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: BitZyon.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: BitZyond.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 44570 or testnet: 44580)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 44571 or testnet: 44581)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  BitZyon is probably already running."), strDataDir.c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "BitZyon server starting\n");
    ...
    if (filesystem::exists(GetDataDir() / strWalletFileName))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("BitZyon"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        }
    ...
    if (mapArgs.count("-reservebalance")) // BitZyon: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // BitZyon: checkpoint master priv key
    {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of BitZyon") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart BitZyon to complete") << "\n";
