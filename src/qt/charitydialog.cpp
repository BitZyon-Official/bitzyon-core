    ...
    model(0)
{
    ...
    ui->charityAddressEdit->setPlaceholderText(tr("Enter BitZyon Address"));
...
//QString charitiesZYONImage[100];
//QString charitiesZYONURL[100];
...
void StakeForCharityDialog::setModel(WalletModel *model)
{
    ...
    qint64 nMin;
    qint64 nMax;
    charitiesThanks[0]="Thank you for donating to the <a href='https://bitzyon.com/ZYONFoundation/' style='color: #1ab06c;'>BitZyon Foundation, Inc.</a>";
    ...
    if (strAddress.IsValid() && nPer > 0 )
    {
        ...
        if (strAddress.ToString().c_str() == QString(FOUNDATION) || strAddress.ToString().c_str() == QString(FOUNDATION_TEST) )
        {
            ...
            ui->message->setText(tr("Thank you for giving to The BitZyon Foundation \n\n"));
        ...
        else
        {
            ...
            if (i==100) // STS ZYON must be a 'manual' entered address
            {
                ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900; }");
        ...
        if (pwalletMain->IsLocked())
        {
            ...
            ui->message->setText(ui->message->text() + "<br />" + "You must unlock your BitZyon software <br />for Staking for Charity to proceed to: " + strAddress.ToString().c_str());
        }
    }
}
...
{   
    ...
    charitiesRequest.setUrl(QUrl(QString("https://bitzyon.com/charities/charities.json")));
    ...
    if (reply->error() == QNetworkReply::NoError) {
        ...
        // clear current combo box entires, leaving the first 2, "manual" and "ZYON foundation"
        for (i=2; i < n; i++ ) {
        ...
        foreach (const QJsonValue &value, json_array) {
            ...
            if (fTestNet) qDebug() << json_obj["ZYONaddress"].toString();
            charitiesAddress[i] = json_obj["ZYONaddress"].toString();
            ...
            //charitiesZYONImage[i] = json_obj["ZYONimg"].toString();
            //charitiesZYONURL[i] = json_obj["ZYONurl"].toString();
            i++;
        }
        ui->comboBox->setItemData(1, "Your donation will be used by the <a href='https://bitzyon.com/ZYONFoundation/' style='color: #ffffff;'>BitZyon Foundation, Inc.</a> <br />under the guidance of the board and community.", Qt::ToolTipRole);
        ui->comboBox->setItemData(0, "You can donate to any BitZyon address you wish.", Qt::ToolTipRole);
    } else {
        uiInterface.ThreadSafeMessageBox("HTTP Error: " + reply->errorString().toStdString(), "BitZyon Dynamic Stake for Charity", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    }
    reply->deleteLater();
}
...
void StakeForCharityDialog::on_enableButton_clicked()
{
    ...
    {
        ui->message->setStyleSheet("QLabel { color: red; font-weight: 900;}");
        ui->message->setText("Please unlock your BitZyon software for sending (not staking only) <br /> for BitZyon Staking for Charity to proceed.");
        return;
    }
    else if (fWalletUnlockStakingOnly)
    {
        ui->message->setStyleSheet("QLabel { color: red; font-weight: 900;}");
        ui->message->setText("Please unlock your BitZyon software for spending <br /> (not staking only) for BitZyon Staking for Charity to proceed.");
        return;
    }
    ...
    ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900;}");
    ui->message->setText("BitZyon Staking For Charity enabled to:<br /> " + QString(address.ToString().c_str()) + " at a rate of " + QString::number(nCharityPercent) + "%");
...
void StakeForCharityDialog::on_disableButton_clicked()
{
    ...
    ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900;}");
    ui->message->setText(tr("BitZyon Dynamic Stake for Charity is now off and saved off."));
    return;
}
...
{
    QPixmap IMGpixmap;
    QPixmap ZYONIMGpixmap;
    ZYONIMGpixmap.load(":/images/Wallet_Logo");
    if (index==0)
    {
        ui->charityAddressEdit->clear();
        ...
        ui->message->setText("Please enter the BitZyon address <br />or select from the drop-down <br />and then click 'Enable'");
        ui->charityAddressEdit->setFocus();
        ui->label_IMG->clear();
        ui->label_HREF->clear();
    }
    else if (index==1)
    {
        ...
        ui->message->setText("Your donation will be used by the <a href='https://bitzyon.com/ZYONFoundation/' style='color: #1ab06c;'>BitZyon Foundation, Inc.</a> <br />under the guidance of the board and community. <br />Click the 'Enable' button above to save <br />and start BitZyon Dynamic Stake for Charity");
        ui->addressBookButton->setDisabled(true);
        ui->charityAddressEdit->setEnabled(false);
        ui->charityAddressEdit->setReadOnly(true);
        ui->label_IMG->setPixmap(ZYONIMGpixmap);
        ui->label_HREF->setText("<a href='https://bitzyon.com/ZYONFoundation/'><span style='text-decoration: underline; color:#1ab06c;'>Learn more</span></a>");
    }
    else if (index > 1)
    {
        ui->charityAddressEdit->clear();
        ...
        ui->charityAddressEdit->setStyleSheet("border-color: #35473c; color:35473c;");
        ui->label_5->setStyleSheet("QLabel {color: #018457;}");
        ui->message->setText(charitiesAsk[index-1].replace("<a href","<a style='color: #1ab06c;' href") + "<br />Click the 'Enable' button above to save <br />and start BitZyon Dynamic Stake for Charity");
...
void StakeForCharityDialog::on_btnRefreshCharities_clicked()
{
    ...
    ui->charityPercentEdit->clear();
    ui->message->setStyleSheet("QLabel { color: #1ab06c; font-weight: 900;}");
    ui->message->setText(tr("Data refreshed from <a href='https://bitzyon.com' style='color: #1ab06c;'>bitzyon.com</a><br />Select a cause you find charitable<br />and click the 'Enable' button to apply changes."));
    ui->comboBox->setCurrentIndex(0); // reset charity select combo
