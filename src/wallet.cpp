...
// BitZyon: total coins staked (non-spendable until maturity)
...
bool CWallet::StakeForCharity()
{
    ...
    for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
    {
        ...
        if (pcoin->IsCoinStake() && pcoin->GetBlocksToMaturity() == 0  && pcoin->GetDepthInMainChain() == nCoinbaseMaturity+10)
        {
            ...
            {
                SendMoneyToDestination(strStakeForCharityAddress.Get(), nNet, wtx, false); // ZYON removed 2 args  , "", true);
...
// Bitzyon: get current stake weight
...
// BitZyon: check 'spent' consistency between wallet and txindex
// BitZyon: fix wallet spent state according to txindex
...
void CWallet::FixSpentCoins(int& nMismatchFound, int64_t& nBalanceInQuestion, int& nOrphansFound, bool fCheckOnly)
{
    ...
    BOOST_FOREACH(CWalletTx* pcoin, vCoins)
    {
        ...
        for (unsigned int n=0; n < pcoin->vout.size(); n++)
        {
            ...
            {
                printf("FixSpentCoins found lost coin %s zyon %s[%d], %s\n",
            ...
            {
                printf("FixSpentCoins found spent coin %s zyon %s[%d], %s\n",
                    FormatMoney(pcoin->vout[n].nValue).c_str(), hash.ToString().c_str(), n, fCheckOnly? "repair not attempted" : "repairing");
                nMismatchFound++;
                nBalanceInQuestion += pcoin->vout[n].nValue;
                if (!fCheckOnly)
...
// BitZyon: disable transaction (only for coinstake)
