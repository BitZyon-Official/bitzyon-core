...
unsigned int nStakeMinAge = 5 * 60; // 5 minutos
...
const string strMessageMagic = "BitZyon Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // BitZyon: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // BitZyon: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    } else if(pindexBest->nHeight <= 1000  && pindexBest->nHeight >= 201 )
    {
        nSubsidy = 0.05 * COIN;
    } else if(pindexBest->nHeight <= 2250004 && pindexBest->nHeight >= 1001 )
    {
        nSubsidy = 0.025 * COIN;
    } else if(pindexBest->nHeight <= 6650012 && pindexBest->nHeight >= 2250005 )
    {
        nSubsidy = 0.01 * COIN;
    } else
    {
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(pindexBest->nHeight <= 600001  && pindexBest->nHeight >= 201){
        nSubsidy = 50 * COIN;
    } else if(pindexBest->nHeight <= 1150002 && pindexBest->nHeight >= 600002){
        nSubsidy = 25 * COIN;
    } else if(pindexBest->nHeight <= 2250004 && pindexBest->nHeight >= 1150003){
        nSubsidy = 13 * COIN;
    } else if(pindexBest->nHeight <= 2800005 && pindexBest->nHeight >= 2250005){
        nSubsidy = 6 * COIN;
    } else if(pindexBest->nHeight <= 3350006 && pindexBest->nHeight >= 2800006){
        nSubsidy = 3 * COIN;
    } else if(pindexBest->nHeight <= 6650012 && pindexBest->nHeight >= 3350007){
        nSubsidy = 2 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
// BitZyon: find last block index up to pindex
...
static unsigned int GetNextTargetRequired_(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // Includes BitZyon fix for wrong retargeting difficulty by Mammix2
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // BitZyon: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BitZyon: clean up wallet after disconnecting coinstake
    BOOST_FOREACH(CTransaction& tx, vtx)
...
*/
    ...
    if (IsProofOfStake())
    {
        // BitZyon: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // BitZyon: track money supply and mint amount info
...
// BitZyon: total coin age spent in transaction, in the unit of coin-days.
...
// BitZyon: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // BitZyon: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // BitZyon: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // BitZyon: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // BitZyon: compute stake modifier
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig, int height) const
{
    ...
    if (IsProofOfStake())
    {
        ...
        // Bitzyon: check proof-of-stake block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // BitZyon: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // BitZyon: ask for pending sync-checkpoint if any
...
printf("ProcessBlock: ORPHAN BLOCK %lu, prev=%s\n", (unsigned long)mapOrphanBlocks.size(), pblock->hashPrevBlock.ToString().substr(0,20).c_str());
  if (pfrom) {
      ...
      // Ask this guy to fill in what we're missing
          ...
          // BitZyon: getblocks may not obtain the ancestor block rejected
    ...
    // BitZyon: if responsible for sync-checkpoint send it
...
// bitzyon: attempt to generate suitable proof-of-stake
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitZyon", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (fTestNet)
    {
        ...
        nStakeMinAge = 5 * 60; // test net min age is 1 hour
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Mineradora brasileira de bitcoin será adquirida por US$30 milhões - BrInvest - 26-08-2020";
        CTransaction txNew;
        txNew.nTime = 1598478788;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1598478788;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1063304;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x0772e1c6be13a935bc53fd14830350e1a75c187b3308f9b8815c005f9d5a5f7f"));
        ...
        // BitZyon: initialize synchronized checkpoint
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // BitZyon: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // BitZyon: tell downloading node about the latest block if it's
